...
class FoodsMapWidget extends StatefulWidget {
  const FoodsMapWidget({
    Key key,
  ...
  @override
  _FoodsMapWidgetState createState() => _FoodsMapWidgetState();
}

class _FoodsMapWidgetState extends State<FoodsMapWidget> {
            ...
            builder: (_, state) {
              return Listener(
                onPointerDown: (_) => context
                    .read<SearchBloc>()
                    .add(const SearchResultSelected(Food.empty)),
                child: g_maps.GoogleMap(
                  onMapCreated: (controller) async {
                    ...
                    context
                        .read<FoodsBloc>()
                        .add(FoodsNearbyRequestedEvent(_center ?? location));
                  ...
                  markers: state.results.map((food) {
                    return g_maps.Marker(
                      markerId: g_maps.MarkerId(food.restaurantId),
                      position: food.latLng.toGMapsLatLng(),
                      onTap: () {
                        context
                            .read<SearchBloc>()
                            .add(SearchResultSelected(food));
                      },
                      icon: food == state.selected ? _selectedMarker : _marker,
                      zIndex: food == state.selected ? 1.0 : 0.0,
                    );
                  }).toSet(),
